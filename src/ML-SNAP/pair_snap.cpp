  ...
  for (int ii = 0; ii < list->inum; ii++) {
    ...
    for (int jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]&&rsq>1e-20) {
        ...
        if (chemflag || chembedflag) snaptr->element[ninside] = jelem;
        ninside++;
      }
    }
    ...
    for (int jj = 0; jj < ninside; jj++) {
      ...
      int jelem=snaptr->element[jj];
      if (chembedflag) {
        double aux;
        double* emb_weights=snaptr->chembed_tau[ielem][jelem];
        int* doit=snaptr->chembed_do[ielem][jelem];
        int until=snaptr->chembed_until[ielem][jelem];
        for (int d = 0; d < until; d++){
          snaptr->compute_deidrj(fij, doit[d]);
          aux=scale[itype][itype]*emb_weights[doit[d]];
          for (int a = 0; a < 3; a++){
            fij[a] = fij[a]*aux;
            f[i][a] += fij[a];
            f[j][a] -= fij[a];
        ...
        if (chemflag)
          snaptr->compute_deidrj(fij, jelem);
        else
          snaptr->compute_deidrj(fij,0);
        
        for (int a = 0; a < 3; a++){
          fij[a]=fij[a]*scale[itype][itype];
          f[i][a] += fij[a];
          f[j][a] -= fij[a];
        }
      }
...
void PairSNAP::coeff(int narg, char **arg)
{
  ...
  snaptr = new SNA(lmp, rfac0, twojmax,
                   ...
                   chemflag, chembeddim, bnormflag,
                   wselfallflag, nelements, switchinnerflag);

  if (chembedflag)
    snaptr->read_chembedfile(chembedfile,elements,NULL);
...
void PairSNAP::read_files(char *coefffilename, char *paramfilename)
{
  ...
  while (true) {
    ...
    if (keywd == "sinner" || keywd == "dinner") {
      ...
      else if (keywd == "chembeddim") {
        chembeddim = utils::inumeric(FLERR,keyval.c_str(),false,lmp);
        chembedflag = 1;
      } else if (keywd == "chembedfile") {
        std::strcpy(chembedfile,keyval.c_str());
        chembedflag = 1;
  ...
  if ((chemflag || chembedflag) && nelemtmp != nelements)
    error->all(FLERR,"Incorrect SNAP parameter file");

	if (chembedflag == 1 && chembeddim == 0)
    error->all(FLERR,"You gave the chemical embedding file but forgot to give the dimension");

  if (chembedflag == 1 && chembedfile[0] == '\0')
    error->all(FLERR,"You gave the chemical embedding dimension but forgot to give the file");
