...

{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg], "rmin0") == 0) {
      ...
      iarg += 2 + ntypes;
    } else if (strcmp(arg[iarg], "chembed") == 0) {
      if (iarg + 3 > narg) error->all(FLERR,"Illegal compute {} command", style);
      chembeddim = utils::inumeric(FLERR, arg[iarg + 1], false, lmp);
      chembedflag = 1;
      std::strcpy(chembedfile,arg[iarg + 2]);
      iarg += 3;
  ...
  if (chembedflag  && !chemflag)
    error->all(
        FLERR,
        "Illegal compute {} command: You want to use chembed but forgot to give the elements' list!",
        style);
  if (chembedflag && chemflag)
    chemflag=0;

  snaptr = new SNA(lmp, rfac0, twojmax, rmin0, switchflag, bzeroflag, chemflag, chembeddim,
                   bnormflag,wselfallflag, nelements, switchinnerflag);

  if (chembedflag)
    snaptr->read_chembedfile(chembedfile,NULL,map);
