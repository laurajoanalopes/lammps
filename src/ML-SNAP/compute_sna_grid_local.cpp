    ...
    ComputeGridLocal(lmp, narg, arg), cutsq(nullptr), radelem(nullptr), wjelem(nullptr)
{
  ...
  if (!switchinnerflag && (sinnerflag || dinnerflag))
    ...
    error->all(FLERR,"You want to use chembed but forgot to give the elements' list!");
  if (chembedflag && chemflag)
    chemflag=0;

  snaptr = new SNA(lmp, rfac0, twojmax, rmin0, switchflag, bzeroflag, chemflag, chembedflag,
										bnormflag, wselfallflag, nelements, switchinnerflag);

	if (chembedflag) snaptr->read_chembedfile(chembedfile,NULL,map);
...
ComputeSNAGridLocal::~ComputeSNAGridLocal()
{
  ...
  if (chemflag || chembedflag) memory->destroy(map);
}
...
void ComputeSNAGridLocal::compute_local()
{
  ...
  for (int iz = nzlo; iz <= nzhi; iz++)
	...
	// not clear what a better choice would be
        ...
        const int itype = 1;
        int ielem = 0;
        if (chemflag || chembedflag) ielem = map[itype];
        ...
        for (int j = 0; j < ntotal; j++) {
          ...
          int jtype = type[j];
          int jelem = 0;
          if (chemflag || chembedflag) jelem = map[jtype];
